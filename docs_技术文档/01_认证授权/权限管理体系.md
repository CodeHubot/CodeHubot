...
async def get_group_devices(
    ...
    if not membership:
        raise HTTPException(status_code=403, detail="不属于此小组")
    
    # 查询小组授权的设备
    authorizations = db.query(PblGroupDeviceAuthorization).filter(
        PblGroupDeviceAuthorization.group_id == group_id,
        PblGroupDeviceAuthorization.is_active == True
    ).all()
    
    return {"devices": [auth.device for auth in authorizations]}
```
...
| API 端点 | 方法 | 管理员 | 渠道 | 教师 | 学生 | 独立用户 |
|---------|------|--------|------|------|------|---------|
| `/api/pbl/courses` | GET | ✅ | ⚠️ | ⚠️ | ⚠️ | ❌ |
| `/api/pbl/courses` | POST | ✅ | ❌ | ✅ | ❌ | ❌ |
| `/api/pbl/courses/{id}` | PUT | ✅ | ❌ | ⚠️ | ❌ | ❌ |
| `/api/pbl/courses/{id}` | DELETE | ✅ | ❌ | ⚠️ | ❌ | ❌ |
