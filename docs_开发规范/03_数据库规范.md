...
-- ✅ PBL 模块表示例(pbl_*)
CREATE TABLE `pbl_courses` (...);               -- 课程表
CREATE TABLE `pbl_students` (...);              -- 学生表
CREATE TABLE `pbl_course_templates` (...);      -- 课程模板表
CREATE TABLE `pbl_unit_templates` (...);        -- 单元模板表
...
-- 扩展表(aiot_*, pbl_*): 可选功能表,可根据业务需求选择性部署
-- - AI 功能(agents, workflows, knowledge_bases)
-- - PBL 教学功能(courses, students, templates)
-- - 其他扩展模块
```
...
CONSTRAINT `fk_courses_school` FOREIGN KEY (`school_id`) 
  REFERENCES `pbl_schools` (`id`) ON DELETE CASCADE
```
...
CREATE TABLE `pbl_courses` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_uuid` (`uuid`),
  KEY `idx_school_id` (`school_id`),           -- 频繁按学校查询
  KEY `idx_status` (`status`),                  -- 频繁按状态筛选
  KEY `idx_created_at` (`created_at`),          -- 频繁按时间排序
  KEY `idx_school_status` (`school_id`, `status`) -- 联合查询优化
...
-- 枚举值说明
`role` VARCHAR(20) COMMENT '角色: admin-管理员, teacher-教师, student-学生'

-- 外键说明
`school_id` BIGINT(20) NOT NULL COMMENT '学校ID(关联 pbl_schools.id)'
```
...
SET @sql = IF(@column_exists = 0,
  'ALTER TABLE `pbl_courses` ADD COLUMN `template_id` BIGINT(20) DEFAULT NULL COMMENT \'课程模板ID\' AFTER `uuid`',
...
ALTER TABLE `pbl_courses` 
ADD COLUMN `template_id` BIGINT(20) DEFAULT NULL COMMENT '课程模板ID' AFTER `uuid`;
 ...
 * 警告: 本脚本仅可执行一次!
 */
ALTER TABLE `pbl_courses` ADD COLUMN `template_id` BIGINT(20);
```
...
-- 查询时过滤已删除数据
SELECT * FROM `pbl_courses` WHERE `is_deleted` = 0;

-- ❌ 物理删除(谨慎使用,仅用于临时数据)
DELETE FROM `pbl_courses` WHERE `id` = 123;
```
...
FOREIGN KEY (`school_id`) REFERENCES `pbl_schools` (`id`)
ON DELETE CASCADE
ON UPDATE CASCADE;
