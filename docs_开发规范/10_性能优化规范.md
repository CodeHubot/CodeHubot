# 性能优化规范

> 本文档定义了 CodeHubot 项目的性能优化规范，包括后端、前端和数据库的优化策略。

## 📋 目录

- [后端性能优化](#后端性能优化)
- [前端性能优化](#前端性能优化)
- [数据库优化](#数据库优化)
- [缓存策略](#缓存策略)
- [性能监控](#性能监控)
- [最佳实践](#最佳实践)

---

## ⚡ 后端性能优化

### 1. 数据库查询优化

#### 避免 N+1 查询

```python
from sqlalchemy.orm import joinedload, selectinload

# ❌ 不好: N+1 查询问题
@router.get("/courses")
def get_courses(db: Session = Depends(get_db)):
    courses = db.query(Course).all()
    
    result = []
    for course in courses:
        # 每次循环都会查询数据库(N+1问题)
        units = course.units  # 触发额外查询
        result.append({
            "course": course.to_dict(),
            "units": [u.to_dict() for u in units]
        })
    
    return success_response(data=result)

# ✅ 好: 使用 joinedload 预加载
@router.get("/courses")
def get_courses(db: Session = Depends(get_db)):
    # 一次性加载课程和单元
    courses = db.query(Course).options(
        joinedload(Course.units)
    ).all()
    
    result = [
        {
            "course": course.to_dict(),
            "units": [u.to_dict() for u in course.units]  # 不触发额外查询
        }
        for course in courses
    ]
    
    return success_response(data=result)

# ✅ 多对多关系使用 selectinload
courses = db.query(Course).options(
    selectinload(Course.students),
    selectinload(Course.units).selectinload(Unit.tasks)
).all()
```

#### 仅查询需要的字段

```python
from sqlalchemy import select

# ❌ 不好: 查询所有字段
courses = db.query(Course).all()

# ✅ 好: 只查询需要的字段
courses = db.query(
    Course.id,
    Course.uuid,
    Course.title,
    Course.status
).all()

# 或使用 with_entities
courses = db.query(Course).with_entities(
    Course.id,
    Course.uuid,
    Course.title
).all()
```

#### 使用分页

```python
# ✅ 始终使用分页,避免一次性加载大量数据
@router.get("/courses")
def get_courses(
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    db: Session = Depends(get_db)
):
    # 计算总数
    total = db.query(Course).count()
    
    # 分页查询
    courses = db.query(Course).offset(
        (page - 1) * page_size
    ).limit(page_size).all()
    
    return success_response(data={
        "items": [c.to_dict() for c in courses],
        "total": total,
        "page": page,
        "page_size": page_size
    })
```

### 2. 使用索引

```python
# 确保常用查询字段有索引
# 在模型中定义索引
class Course(Base):
    __tablename__ = "aiot_agents"
    
    id = Column(BigInteger, primary_key=True, index=True)
    uuid = Column(String(36), unique=True, index=True)
    school_id = Column(BigInteger, index=True)  # 外键索引
    status = Column(String(20), index=True)     # 频繁筛选字段
    created_at = Column(DateTime, index=True)    # 排序字段
    
    # 联合索引
    __table_args__ = (
        Index('idx_school_status', 'school_id', 'status'),
    )
```

### 3. 异步任务

```python
from celery import Celery

celery_app = Celery('tasks', broker='redis://localhost:6379/0')

# ✅ 耗时操作使用异步任务
@celery_app.task
def process_video_thumbnail(video_id: int):
    """生成视频缩略图(耗时操作)"""
    # 处理逻辑
    pass

@celery_app.task
def send_email_notification(user_id: int, subject: str, content: str):
    """发送邮件(耗时操作)"""
    # 发送逻辑
    pass

# API 中调用异步任务
@router.post("/videos")
def upload_video(video: UploadFile, db: Session = Depends(get_db)):
    # 保存视频
    video_record = Video(filename=video.filename)
    db.add(video_record)
    db.commit()
    
    # ✅ 异步生成缩略图,不阻塞响应
    process_video_thumbnail.delay(video_record.id)
    
    return success_response(data=video_record.to_dict())
```

### 4. 响应数据优化

```python
# ❌ 不好: 返回完整的嵌套对象
@router.get("/courses/{uuid}")
def get_course(uuid: str, db: Session = Depends(get_db)):
    course = db.query(Course).options(
        joinedload(Course.units).joinedload(Unit.tasks),
        joinedload(Course.students)
    ).filter(Course.uuid == uuid).first()
    
    # 包含大量嵌套数据,响应体很大
    return success_response(data=course.to_dict(include_all=True))

# ✅ 好: 分级返回数据
@router.get("/courses/{uuid}")
def get_course_summary(uuid: str, db: Session = Depends(get_db)):
    """课程概要(不包含详细内容)"""
    course = db.query(Course).filter(Course.uuid == uuid).first()
    return success_response(data=course.to_dict())

@router.get("/courses/{uuid}/detail")
def get_course_detail(uuid: str, db: Session = Depends(get_db)):
    """课程详情(包含单元列表)"""
    course = db.query(Course).options(
        joinedload(Course.units)
    ).filter(Course.uuid == uuid).first()
    
    return success_response(data={
        **course.to_dict(),
        "units": [u.to_dict() for u in course.units]
    })

@router.get("/courses/{uuid}/units/{unit_id}/tasks")
def get_unit_tasks(uuid: str, unit_id: int, db: Session = Depends(get_db)):
    """按需加载任务列表"""
    tasks = db.query(Task).filter(Task.unit_id == unit_id).all()
    return success_response(data=[t.to_dict() for t in tasks])
```

---

## 🚀 前端性能优化

### 1. 路由懒加载

```javascript
// router/index.js

// ❌ 不好: 直接导入(打包到主bundle)
import AgentList from '@/modules/ai/views/AgentList.vue'
import AgentDetail from '@/modules/ai/views/AgentDetail.vue'

// ✅ 好: 懒加载(按需加载)
const routes = [
  {
    path: '/courses',
    name: 'CourseList',
    component: () => import('@/modules/ai/views/AgentList.vue')
  },
  {
    path: '/courses/:uuid',
    name: 'CourseDetail',
    component: () => import('@/modules/ai/views/AgentDetail.vue')
  }
]
```

### 2. 组件按需导入

```javascript
// ❌ 不好: 导入整个 Element Plus
import ElementPlus from 'element-plus'
import 'element-plus/dist/index.css'
app.use(ElementPlus)

// ✅ 好: 按需导入组件
import { ElButton, ElTable, ElMessage } from 'element-plus'
app.component('ElButton', ElButton)
app.component('ElTable', ElTable)
```

### 3. 虚拟滚动(大列表优化)

```vue
<template>
  <!-- ❌ 不好: 渲染所有数据(10000条) -->
  <div class="list">
    <div v-for="item in items" :key="item.id" class="item">
      {{ item.name }}
    </div>
  </div>
  
  <!-- ✅ 好: 使用虚拟滚动 -->
  <el-table-v2
    :columns="columns"
    :data="items"
    :width="700"
    :height="400"
    fixed
  />
</template>

<script setup>
// 或使用 vue-virtual-scroller
import { RecycleScroller } from 'vue-virtual-scroller'
import 'vue-virtual-scroller/dist/vue-virtual-scroller.css'
</script>
```

### 4. 图片懒加载

```vue
<template>
  <div class="course-list">
    <!-- ✅ 使用 v-lazy 或 loading="lazy" -->
    <img
      v-for="course in courses"
      :key="course.id"
      v-lazy="course.cover_image"
      alt="课程封面"
    />
    
    <!-- 原生懒加载 -->
    <img
      :src="course.cover_image"
      loading="lazy"
      alt="课程封面"
    />
  </div>
</template>

<script setup>
// 使用 vue-lazyload
import VueLazyload from 'vue-lazyload'
app.use(VueLazyload, {
  preLoad: 1.3,
  loading: '/loading.gif',
  error: '/error.png'
})
</script>
```

### 5. 防抖和节流

```vue
<template>
  <div>
    <!-- 搜索框 - 使用防抖 -->
    <el-input
      v-model="searchKeyword"
      @input="handleSearchDebounced"
      placeholder="搜索课程"
    />
    
    <!-- 滚动加载 - 使用节流 -->
    <div @scroll="handleScrollThrottled">
      <!-- 内容 -->
    </div>
  </div>
</template>

<script setup>
import { ref } from 'vue'
import { debounce, throttle } from 'lodash-es'

const searchKeyword = ref('')

// ✅ 防抖: 用户停止输入后才执行
const handleSearchDebounced = debounce((value) => {
  console.log('搜索:', value)
  // 执行搜索
}, 500)

// ✅ 节流: 限制执行频率
const handleScrollThrottled = throttle(() => {
  console.log('滚动事件')
  // 加载更多
}, 200)
</script>
```

### 6. 计算属性缓存

```vue
<script setup>
import { ref, computed } from 'vue'

const courses = ref([...])

// ✅ 好: 使用计算属性(有缓存)
const activeCourses = computed(() => {
  return courses.value.filter(c => c.status === 'active')
})

const totalCount = computed(() => {
  return courses.value.length
})

// ❌ 不好: 使用方法(每次调用都计算)
function getActiveCourses() {
  return courses.value.filter(c => c.status === 'active')
}
</script>
```

### 7. Keep-Alive 缓存组件

```vue
<template>
  <router-view v-slot="{ Component }">
    <!-- ✅ 缓存列表页面,避免重复请求 -->
    <keep-alive :include="['CourseList', 'StudentList']">
      <component :is="Component" />
    </keep-alive>
  </router-view>
</template>
```

---

## 🗄️ 数据库优化

### 1. 索引优化

```sql
-- ✅ 为常用查询添加索引
-- 1. 外键索引
ALTER TABLE `aiot_agents` ADD INDEX `idx_school_id` (`school_id`);

-- 2. 状态筛选索引
ALTER TABLE `aiot_agents` ADD INDEX `idx_status` (`status`);

-- 3. 排序字段索引
ALTER TABLE `aiot_agents` ADD INDEX `idx_created_at` (`created_at`);

-- 4. 联合索引(注意顺序)
ALTER TABLE `aiot_agents` 
ADD INDEX `idx_school_status_created` (`school_id`, `status`, `created_at`);

-- 查看索引使用情况
EXPLAIN SELECT * FROM aiot_agents 
WHERE school_id = 1 AND status = 'published' 
ORDER BY created_at DESC;
```

### 2. 查询优化

```sql
-- ❌ 不好: SELECT *
SELECT * FROM aiot_agents WHERE school_id = 1;

-- ✅ 好: 只查询需要的字段
SELECT id, uuid, title, status, created_at 
FROM aiot_agents 
WHERE school_id = 1;

-- ❌ 不好: 不使用索引的查询
SELECT * FROM aiot_agents WHERE title LIKE '%Python%';

-- ✅ 好: 前缀匹配可以使用索引
SELECT * FROM aiot_agents WHERE title LIKE 'Python%';

-- ❌ 不好: OR 查询(可能不使用索引)
SELECT * FROM aiot_agents 
WHERE school_id = 1 OR status = 'published';

-- ✅ 好: 使用 UNION(如果合适)
SELECT * FROM aiot_agents WHERE school_id = 1
UNION
SELECT * FROM aiot_agents WHERE status = 'published';
```

### 3. 批量操作

```python
# ❌ 不好: 循环插入(N 次数据库操作)
for student in student_list:
    new_student = Student(name=student['name'], email=student['email'])
    db.add(new_student)
    db.commit()  # 每次都 commit

# ✅ 好: 批量插入(1 次数据库操作)
students = [
    Student(name=s['name'], email=s['email'])
    for s in student_list
]
db.bulk_save_objects(students)
db.commit()

# 或使用 add_all
db.add_all(students)
db.commit()
```

### 4. 连接池配置

```python
from sqlalchemy import create_engine
from sqlalchemy.pool import QueuePool

# 配置数据库连接池
engine = create_engine(
    DATABASE_URL,
    poolclass=QueuePool,
    pool_size=20,          # 连接池大小
    max_overflow=10,       # 最大溢出连接数
    pool_timeout=30,       # 获取连接超时时间
    pool_recycle=3600,     # 连接回收时间(1小时)
    pool_pre_ping=True,    # 连接前 ping,确保连接有效
    echo=False             # 生产环境关闭 SQL 日志
)
```

---

## 💾 缓存策略

### 1. Redis 缓存

```python
import redis
import json
from functools import wraps

# Redis 客户端
redis_client = redis.Redis(host='localhost', port=6379, db=0, decode_responses=True)

def cache_result(expire_seconds=300):
    """缓存装饰器"""
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            # 生成缓存 key
            cache_key = f"{func.__name__}:{args}:{kwargs}"
            
            # 尝试从缓存获取
            cached = redis_client.get(cache_key)
            if cached:
                return json.loads(cached)
            
            # 执行函数
            result = func(*args, **kwargs)
            
            # 存入缓存
            redis_client.setex(
                cache_key,
                expire_seconds,
                json.dumps(result)
            )
            
            return result
        return wrapper
    return decorator

# 使用缓存
@cache_result(expire_seconds=600)  # 缓存 10 分钟
def get_course_statistics(school_id: int):
    """获取学校课程统计(耗时查询)"""
    # 复杂的统计查询
    return db.query(...).all()
```

### 2. 应用层缓存

```python
from functools import lru_cache

# ✅ 使用 LRU 缓存配置数据
@lru_cache(maxsize=128)
def get_system_config(config_key: str) -> str:
    """获取系统配置(不常变化)"""
    config = db.query(SystemConfig).filter(
        SystemConfig.config_key == config_key
    ).first()
    return config.config_value if config else None

# 使用
max_upload_size = get_system_config('max_upload_size')
```

### 3. 前端缓存

```javascript
// 使用 localStorage 缓存静态数据
const CACHE_KEY = 'course_categories'
const CACHE_EXPIRE = 1000 * 60 * 60  // 1小时

export async function getCourseCategories() {
  // 检查缓存
  const cached = localStorage.getItem(CACHE_KEY)
  if (cached) {
    const data = JSON.parse(cached)
    if (Date.now() - data.timestamp < CACHE_EXPIRE) {
      return data.value
    }
  }
  
  // 请求数据
  const result = await request({
    url: '/api/course-categories',
    method: 'get'
  })
  
  // 存入缓存
  localStorage.setItem(CACHE_KEY, JSON.stringify({
    value: result,
    timestamp: Date.now()
  }))
  
  return result
}
```

---

## 📊 性能监控

### 1. API 响应时间监控

```python
import time
import logging
from fastapi import Request

logger = logging.getLogger(__name__)

@app.middleware("http")
async def log_requests(request: Request, call_next):
    """记录请求响应时间"""
    start_time = time.time()
    
    response = await call_next(request)
    
    process_time = time.time() - start_time
    
    # 记录慢查询(超过1秒)
    if process_time > 1.0:
        logger.warning(
            f"慢查询: {request.method} {request.url.path} "
            f"耗时: {process_time:.2f}秒"
        )
    
    # 添加响应头
    response.headers["X-Process-Time"] = str(process_time)
    
    return response
```

### 2. 数据库查询监控

```python
from sqlalchemy import event
from sqlalchemy.engine import Engine
import logging

logger = logging.getLogger(__name__)

@event.listens_for(Engine, "before_cursor_execute")
def before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
    conn.info.setdefault('query_start_time', []).append(time.time())

@event.listens_for(Engine, "after_cursor_execute")
def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
    total_time = time.time() - conn.info['query_start_time'].pop()
    
    # 记录慢查询(超过0.5秒)
    if total_time > 0.5:
        logger.warning(f"慢查询 ({total_time:.2f}s): {statement}")
```

### 3. 前端性能监控

```javascript
// 使用 Performance API
window.addEventListener('load', () => {
  const perfData = window.performance.timing
  const pageLoadTime = perfData.loadEventEnd - perfData.navigationStart
  const connectTime = perfData.responseEnd - perfData.requestStart
  
  console.log('页面加载时间:', pageLoadTime, 'ms')
  console.log('请求响应时间:', connectTime, 'ms')
  
  // 上报到监控平台
  // reportToMonitoring({ pageLoadTime, connectTime })
})

// Vue Router 导航性能
router.beforeEach((to, from, next) => {
  performance.mark('route-start')
  next()
})

router.afterEach(() => {
  performance.mark('route-end')
  performance.measure('route-duration', 'route-start', 'route-end')
  
  const measure = performance.getEntriesByName('route-duration')[0]
  console.log('路由切换耗时:', measure.duration, 'ms')
})
```

---

## 💡 最佳实践

### 1. 性能优化检查清单

```markdown
## 后端

- [ ] 数据库查询使用索引
- [ ] 避免 N+1 查询问题
- [ ] 使用分页,不一次性加载大量数据
- [ ] 耗时操作使用异步任务
- [ ] 热点数据使用缓存
- [ ] 配置数据库连接池
- [ ] 监控慢查询并优化

## 前端

- [ ] 路由和组件懒加载
- [ ] 大列表使用虚拟滚动
- [ ] 图片使用懒加载
- [ ] 搜索使用防抖,滚动使用节流
- [ ] 组件使用 Keep-Alive 缓存
- [ ] 按需导入第三方库
- [ ] 优化打包体积

## 数据库

- [ ] 常用查询字段有索引
- [ ] 避免 SELECT *
- [ ] 使用批量操作代替循环操作
- [ ] 定期分析和优化慢查询
```

### 2. 性能测试

```bash
# 后端 API 压力测试
ab -n 1000 -c 10 http://localhost:8000/api/courses

# 使用 locust 进行压力测试
locust -f locustfile.py --host=http://localhost:8000

# 前端性能测试
lighthouse https://yourdomain.com
```

---

## 📚 参考资源

- [FastAPI 性能优化](https://fastapi.tiangolo.com/deployment/concepts/)
- [Vue.js 性能优化](https://vuejs.org/guide/best-practices/performance.html)
- [MySQL 性能优化](https://dev.mysql.com/doc/refman/8.0/en/optimization.html)
- [Web Performance](https://web.dev/performance/)

---

**记住**: 过早优化是万恶之源,但合理的性能优化能显著提升用户体验！先测量,再优化!
