# Git 工作流规范

> 本文档定义了 CodeHubot 项目的 Git 使用规范，包括提交信息、分支管理和协作流程。

## 📋 目录

- [提交信息规范](#提交信息规范)
- [分支管理策略](#分支管理策略)
- [提交流程](#提交流程)
- [代码审查](#代码审查)
- [最佳实践](#最佳实践)

---

## 📝 提交信息规范

### 1. 提交信息格式

使用 **Conventional Commits** 规范：

```bash
<type>(<scope>): <subject>

<body>

<footer>
```

#### 简化格式（推荐）

```bash
<type>(<scope>): <subject>

# 示例:
feat(ai): 添加智能体管理功能
fix(api): 修复用户登录token过期问题
docs(readme): 更新部署文档
```

### 2. Type 类型

| Type | 说明 | 示例 |
|------|------|------|
| **feat** | 新功能 | feat(ai): 添加智能体管理 |
| **fix** | 修复bug | fix(api): 修复分页查询错误 |
| **docs** | 文档更新 | docs(api): 更新API接口文档 |
| **style** | 代码格式调整（不影响功能） | style(vue): 统一代码缩进格式 |
| **refactor** | 重构（不是新功能也不是修复bug） | refactor(auth): 优化权限检查逻辑 |
| **perf** | 性能优化 | perf(query): 优化课程列表查询 |
| **test** | 测试相关 | test(api): 添加课程API测试 |
| **chore** | 构建/工具链相关 | chore(deps): 升级依赖包版本 |
| **revert** | 回滚提交 | revert: 回滚feat(ai): xxx |

### 3. Scope 范围

常用的 scope:

```bash
# 后端模块
api         # API 接口
ai          # AI 模块
auth        # 认证模块
device      # 设备模块
ai          # AI 模块
db          # 数据库

# 前端模块
vue         # Vue 组件
router      # 路由
ui          # UI 组件
style       # 样式

# 其他
docs        # 文档
config      # 配置
deploy      # 部署
sql         # SQL 脚本
```

### 4. Subject 主题

- 使用中文描述（清晰易懂）
- 简洁明了（不超过 50 字符）
- 不以句号结尾
- 使用祈使句（"添加"而不是"已添加"）

#### ✅ 好的示例

```bash
feat(ai): 添加智能体管理功能
fix(api): 修复用户登录token过期导致的401错误
docs(readme): 更新Docker部署说明
refactor(auth): 简化权限检查逻辑
perf(query): 优化课程列表查询性能
```

#### ❌ 不好的示例

```bash
update code                           # 太模糊
修复bug                               # 没有具体说明
feat: 添加了一个新功能用于课程模板管理  # 太长
Fix Bug.                              # 英文不规范，有句号
```

### 5. Body 正文（可选）

详细说明修改内容、原因和影响：

```bash
feat(ai): 添加智能体管理功能

- 新增智能体配置表 aiot_agents
- 实现平台管理员分配模板权限给学校
- 学校管理员可查看可用模板列表
- 支持按难度、类别筛选模板

数据库变更: SQL/update/32_add_template_permissions.sql
```

### 6. Footer 脚注（可选）

用于关联 Issue 或标注 Breaking Changes：

```bash
fix(api): 修复分页查询bug

修复了查询参数验证逻辑

Closes #123
Fixes #456

# Breaking Change 示例
feat(api): 重构API响应格式

BREAKING CHANGE: 所有API响应格式改为统一格式 {code, message, data}
```

---

## 🌲 分支管理策略

### 1. 主分支

```bash
main (或 master)
├── 生产环境代码
├── 受保护分支（不能直接提交）
└── 所有功能最终合并到这里
```

### 2. 简化的分支策略

由于项目规模和团队特点，建议使用简化的分支策略：

#### 方式 1: 直接在 main 开发（小团队）

```bash
# 适用场景: 1-3人团队，开发节奏快

# 1. 更新代码
git pull origin main

# 2. 开发和提交
git add .
git commit -m "feat(ai): 添加新功能"

# 3. 推送
git push origin main
```

#### 方式 2: 临时功能分支（推荐）

```bash
# 适用场景: 多人协作，功能开发时间较长

# 1. 创建功能分支
git checkout -b feat/template-permissions

# 2. 开发和提交
git add .
git commit -m "feat(ai): 添加智能体管理"

# 3. 推送到远程
git push origin feat/template-permissions

# 4. 合并到 main（本地或通过 PR）
git checkout main
git pull origin main
git merge feat/template-permissions
git push origin main

# 5. 删除功能分支
git branch -d feat/template-permissions
git push origin --delete feat/template-permissions
```

### 3. 分支命名规范

```bash
# 功能分支
feat/template-permissions     # 新功能
feat/course-analytics         # 新功能

# 修复分支
fix/login-token-expire        # bug修复
fix/pagination-error          # bug修复

# 重构分支
refactor/auth-system          # 代码重构

# 文档分支
docs/api-specification        # 文档更新

# 示例:
git checkout -b feat/student-portfolio
git checkout -b fix/course-list-pagination
git checkout -b docs/development-guide
```

---

## 🔄 提交流程

### 1. 日常开发流程

```bash
# 1. 更新本地代码
git checkout main
git pull origin main

# 2. 创建功能分支（可选，复杂功能推荐）
git checkout -b feat/new-feature

# 3. 开发和测试
# ... 编写代码 ...

# 4. 查看修改
git status
git diff

# 5. 暂存修改
git add .
# 或选择性添加
git add backend/app/api/ai/agents.py
git add frontend/src/modules/ai/views/AgentList.vue

# 6. 提交
git commit -m "feat(ai): 添加智能体管理功能"

# 7. 推送
git push origin feat/new-feature
# 或直接推送到 main
git push origin main

# 8. 合并到 main（如果使用了功能分支）
git checkout main
git merge feat/new-feature
git push origin main

# 9. 删除功能分支
git branch -d feat/new-feature
```

### 2. 提交前检查清单

```bash
# ✅ 提交前必须检查:

# 1. 代码格式正确
# 2. 没有语法错误
# 3. 没有 console.log 等调试代码
# 4. 没有硬编码的敏感信息
# 5. API 使用统一响应格式
# 6. 数据库变更已生成 SQL 脚本
# 7. 提交信息符合规范
# 8. 不包含无关的文件修改

# 查看将要提交的内容
git diff --staged
```

### 3. 修改提交信息

```bash
# 修改最后一次提交信息
git commit --amend -m "feat(ai): 修正后的提交信息"

# 修改多次提交（交互式rebase）
git rebase -i HEAD~3  # 修改最近3次提交

# ⚠️ 注意: 不要修改已经推送的提交
```

### 4. 撤销操作

```bash
# 撤销工作区修改（未 add）
git checkout -- filename

# 撤销暂存区修改（已 add，未 commit）
git reset HEAD filename

# 撤销提交（已 commit，未 push）
git reset --soft HEAD~1     # 保留修改
git reset --hard HEAD~1     # 丢弃修改

# 撤销推送的提交（已 push）
git revert <commit-hash>    # 创建新提交来撤销
```

---

## 👥 代码审查

### 1. Pull Request 流程（可选）

```bash
# 1. 推送功能分支
git push origin feat/new-feature

# 2. 在 GitHub/GitLab 创建 PR
# - 填写 PR 标题和描述
# - 关联相关 Issue
# - 请求代码审查

# 3. 代码审查
# - 审查者检查代码质量
# - 提出修改建议
# - 讨论实现方案

# 4. 修改和更新
git add .
git commit -m "fix: 根据审查意见修改"
git push origin feat/new-feature

# 5. 合并 PR
# - 审查通过后合并到 main
# - 删除功能分支
```

### 2. 代码审查重点

```markdown
# 代码审查检查清单:

## 功能性
- [ ] 实现了需求的功能
- [ ] 没有明显的 bug
- [ ] 边界情况处理正确

## 代码质量
- [ ] 代码逻辑清晰易懂
- [ ] 命名规范，见名知意
- [ ] 没有重复代码
- [ ] 函数职责单一

## 规范性
- [ ] 符合代码风格规范
- [ ] API 使用统一响应格式
- [ ] 错误处理完整
- [ ] 有必要的注释

## 安全性
- [ ] 没有 SQL 注入风险
- [ ] 没有 XSS 风险
- [ ] 敏感信息已加密
- [ ] 权限检查完整

## 性能
- [ ] 没有明显性能问题
- [ ] 数据库查询优化
- [ ] 合理使用索引
```

---

## 💡 最佳实践

### 1. 提交粒度

```bash
# ✅ 好的提交: 一个提交只做一件事
git commit -m "feat(ai): 添加智能体列表API"
git commit -m "feat(ai): 添加智能体详情API"
git commit -m "style(ai): 优化智能体列表页面样式"

# ❌ 不好的提交: 一个提交做多件事
git commit -m "添加课程模板功能并修复登录bug"  # 应该拆分
```

### 2. 提交频率

```bash
# ✅ 推荐: 完成一个小功能就提交
# - 添加一个 API 接口
# - 完成一个页面
# - 修复一个 bug

# ❌ 避免: 
# - 几天不提交，一次性提交大量代码
# - 每改一行就提交
```

### 3. 分支管理

```bash
# ✅ 及时删除已合并的分支
git branch -d feat/old-feature
git push origin --delete feat/old-feature

# ✅ 保持分支简洁
git branch  # 定期查看本地分支
git branch -a  # 查看所有分支（包括远程）

# ❌ 避免分支过多导致混乱
# 项目要求: 不要创建太多长期分支
```

### 4. 冲突解决

```bash
# 更新代码时遇到冲突
git pull origin main

# 手动解决冲突
# 1. 编辑冲突文件
# 2. 删除冲突标记 (<<<<, ====, >>>>)
# 3. 测试确保功能正常

# 标记冲突已解决
git add .
git commit -m "fix: 解决合并冲突"

# 推送
git push origin main
```

### 5. .gitignore 规范

```bash
# Python
__pycache__/
*.py[cod]
*.so
.env
venv/
*.log

# Node.js
node_modules/
dist/
.DS_Store

# IDE
.vscode/
.idea/
*.swp

# 项目特定
logs/
uploads/
*.db
```

---

## 🚨 注意事项

### 1. 禁止操作

```bash
# ❌ 禁止: 直接修改已推送的历史记录
git push -f origin main  # 危险！会覆盖他人代码

# ❌ 禁止: 提交敏感信息
git add .env             # 包含密码、密钥
git add config.py        # 包含敏感配置

# ❌ 禁止: 提交大文件
git add large-file.zip   # 超过 100MB

# ❌ 禁止: 提交临时文件
git add *.log
git add __pycache__/
```

### 2. 保护措施

```bash
# 检查将要提交的文件
git status
git diff --staged

# 移除误添加的文件
git reset HEAD .env

# 删除 Git 历史中的敏感文件
git filter-branch --force --index-filter \
  "git rm --cached --ignore-unmatch .env" \
  --prune-empty --tag-name-filter cat -- --all
```

---

## 📚 参考资源

- [Conventional Commits 规范](https://www.conventionalcommits.org/)
- [Git 官方文档](https://git-scm.com/doc)
- [GitHub Flow](https://guides.github.com/introduction/flow/)
- 项目文档: `GIT_WORKFLOW.md`

---

## 🔧 常用命令速查

```bash
# 查看状态
git status

# 查看日志
git log --oneline --graph --all

# 查看某个文件的修改历史
git log -p filename

# 查看某次提交的详情
git show <commit-hash>

# 查看远程仓库
git remote -v

# 更新远程分支列表
git fetch --prune

# 清理本地已删除的远程分支
git remote prune origin

# 查看配置
git config --list
```

---

**记住**: 规范的 Git 使用习惯让团队协作更顺畅，代码历史更清晰！
