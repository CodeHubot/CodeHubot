# 错误处理规范

> 本文档定义了 CodeHubot 项目的错误处理规范，包括后端异常处理和前端错误提示。

## 📋 目录

- [后端错误处理](#后端错误处理)
- [前端错误处理](#前端错误处理)
- [错误码定义](#错误码定义)
- [错误日志记录](#错误日志记录)
- [最佳实践](#最佳实践)

---

## 🔧 后端错误处理

### 1. 统一异常处理

```python
from fastapi import HTTPException, Request
from fastapi.responses import JSONResponse
from sqlalchemy.exc import IntegrityError
import logging

logger = logging.getLogger(__name__)

# ✅ 在 main.py 中注册全局异常处理器
from fastapi import FastAPI

app = FastAPI()

@app.exception_handler(HTTPException)
async def http_exception_handler(request: Request, exc: HTTPException):
    """处理 HTTP 异常"""
    return JSONResponse(
        status_code=exc.status_code,
        content={
            "code": exc.status_code,
            "message": exc.detail,
            "data": None
        }
    )

@app.exception_handler(Exception)
async def general_exception_handler(request: Request, exc: Exception):
    """处理所有未捕获的异常"""
    logger.error(f"未处理的异常: {str(exc)}", exc_info=True)
    return JSONResponse(
        status_code=500,
        content={
            "code": 500,
            "message": "服务器内部错误",
            "data": None
        }
    )
```

### 2. 使用 HTTPException

```python
from fastapi import HTTPException, Depends
from sqlalchemy.orm import Session

@router.get("/courses/{course_uuid}")
def get_course(
    course_uuid: str,
    db: Session = Depends(get_db)
):
    """获取课程详情"""
    
    # 查询课程
    course = db.query(Course).filter(Course.uuid == course_uuid).first()
    
    # ✅ 资源不存在 - 404
    if not course:
        raise HTTPException(
            status_code=404,
            detail="课程不存在"
        )
    
    # ✅ 软删除检查
    if course.is_deleted:
        raise HTTPException(
            status_code=404,
            detail="课程已被删除"
        )
    
    return success_response(data=course.to_dict())

@router.post("/courses")
def create_course(
    course: CourseCreate,
    current_user = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """创建课程"""
    
    # ✅ 权限检查 - 403
    if current_user.role not in ['admin', 'teacher']:
        raise HTTPException(
            status_code=403,
            detail="无权创建课程"
        )
    
    # ✅ 参数验证 - 400
    if len(course.title) < 2:
        raise HTTPException(
            status_code=400,
            detail="课程标题至少2个字符"
        )
    
    # 创建课程
    try:
        new_course = Course(**course.dict())
        db.add(new_course)
        db.commit()
        return success_response(data=new_course.to_dict())
    except Exception as e:
        db.rollback()
        logger.error(f"创建课程失败: {str(e)}")
        raise HTTPException(status_code=500, detail="创建失败")
```

### 3. 数据库异常处理

```python
from sqlalchemy.exc import IntegrityError, SQLAlchemyError

@router.post("/courses")
def create_course(course: CourseCreate, db: Session = Depends(get_db)):
    try:
        new_course = Course(**course.dict())
        db.add(new_course)
        db.commit()
        db.refresh(new_course)
        
        return success_response(data=new_course.to_dict())
        
    except IntegrityError as e:
        # ✅ 数据库约束错误(唯一键、外键等)
        db.rollback()
        logger.warning(f"数据库约束错误: {str(e)}")
        
        # 判断具体错误类型
        error_msg = str(e.orig)
        if "Duplicate entry" in error_msg:
            raise HTTPException(status_code=400, detail="课程名称已存在")
        elif "foreign key constraint" in error_msg:
            raise HTTPException(status_code=400, detail="关联数据不存在")
        else:
            raise HTTPException(status_code=400, detail="数据约束错误")
            
    except SQLAlchemyError as e:
        # ✅ 其他数据库错误
        db.rollback()
        logger.error(f"数据库错误: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail="数据库操作失败")
        
    except Exception as e:
        # ✅ 其他未知错误
        db.rollback()
        logger.error(f"未知错误: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail="操作失败")
```

### 4. 业务错误处理

```python
from app.core.response import error_response

@router.post("/courses/{course_uuid}/publish")
def publish_course(
    course_uuid: str,
    db: Session = Depends(get_db)
):
    """发布课程"""
    
    course = db.query(Course).filter(Course.uuid == course_uuid).first()
    
    if not course:
        raise HTTPException(status_code=404, detail="课程不存在")
    
    # ✅ 业务逻辑验证
    if course.status == "published":
        # 使用业务错误码(不是 HTTP 错误)
        return error_response(
            code=4001,
            message="课程已发布，无需重复操作",
            status_code=400
        )
    
    if not course.units or len(course.units) == 0:
        return error_response(
            code=4002,
            message="课程至少需要一个单元才能发布",
            status_code=400
        )
    
    # 发布课程
    course.status = "published"
    course.published_at = datetime.now()
    db.commit()
    
    return success_response(message="发布成功")
```

### 5. 参数验证错误

```python
from pydantic import BaseModel, Field, validator
from fastapi import HTTPException

class CourseCreate(BaseModel):
    title: str = Field(..., min_length=2, max_length=200)
    description: str = Field(None, max_length=2000)
    status: str = "draft"
    
    @validator('status')
    def validate_status(cls, v):
        """✅ 自定义验证器"""
        allowed = ['draft', 'published', 'archived']
        if v not in allowed:
            raise ValueError(f'状态必须是: {", ".join(allowed)}')
        return v
    
    @validator('title')
    def validate_title(cls, v):
        """✅ 验证标题"""
        if not v or v.strip() == "":
            raise ValueError('标题不能为空')
        if len(v.strip()) < 2:
            raise ValueError('标题至少2个字符')
        return v.strip()

# FastAPI 会自动捕获 Pydantic 验证错误并返回 422
```

---

## 🌐 前端错误处理

### 1. 统一请求拦截器

```javascript
// utils/request.js
import axios from 'axios'
import { ElMessage } from 'element-plus'
import router from '@/router'

const service = axios.create({
  baseURL: '/api',
  timeout: 30000
})

// 响应拦截器
service.interceptors.response.use(
  response => {
    // ✅ 成功响应 (HTTP 200)
    const { code, message, data } = response.data
    
    if (code === 200) {
      return data  // 直接返回 data
    } else {
      // 业务错误
      ElMessage.error(message || '操作失败')
      return Promise.reject(new Error(message))
    }
  },
  error => {
    // ✅ HTTP 错误处理
    if (error.response) {
      const { status, data } = error.response
      
      switch (status) {
        case 400:
          ElMessage.error(data.message || '请求参数错误')
          break
          
        case 401:
          // 未登录或 token 过期
          ElMessage.error('登录已过期，请重新登录')
          localStorage.removeItem('token')
          router.push('/login')
          break
          
        case 403:
          ElMessage.error(data.message || '无权访问')
          break
          
        case 404:
          ElMessage.error(data.message || '资源不存在')
          break
          
        case 500:
          ElMessage.error(data.message || '服务器错误')
          break
          
        default:
          ElMessage.error(data.message || '未知错误')
      }
    } else if (error.request) {
      // ✅ 网络错误(无响应)
      ElMessage.error('网络连接失败，请检查网络')
    } else {
      // ✅ 请求配置错误
      ElMessage.error('请求失败: ' + error.message)
    }
    
    return Promise.reject(error)
  }
)

export default service
```

### 2. 组件中的错误处理

```vue
<script setup>
import { ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { getCourses, deleteCourse } from '@/modules/pbl/api/courses'

const loading = ref(false)
const courses = ref([])

// ✅ 加载数据 - 基础错误处理
async function loadCourses() {
  try {
    loading.value = true
    const data = await getCourses({ page: 1, pageSize: 20 })
    courses.value = data.items
  } catch (error) {
    // 错误已在 request 中统一处理，这里可以记录日志
    console.error('加载课程失败:', error)
    // 可选：显示额外提示
    // ElMessage.error('加载课程失败，请稍后重试')
  } finally {
    loading.value = false
  }
}

// ✅ 删除操作 - 带确认的错误处理
async function handleDelete(courseId) {
  try {
    // 确认对话框
    await ElMessageBox.confirm(
      '确定要删除这个课程吗？',
      '警告',
      { type: 'warning' }
    )
    
    // 执行删除
    await deleteCourse(courseId)
    ElMessage.success('删除成功')
    
    // 刷新列表
    loadCourses()
    
  } catch (error) {
    if (error === 'cancel') {
      // 用户取消操作
      return
    }
    console.error('删除失败:', error)
  }
}

// ✅ 表单提交 - 完整错误处理
async function handleSubmit() {
  try {
    // 1. 表单验证
    await formRef.value.validate()
    
    // 2. 提交数据
    loading.value = true
    await createCourse(formData.value)
    
    // 3. 成功提示
    ElMessage.success('创建成功')
    
    // 4. 后续操作
    dialogVisible.value = false
    loadCourses()
    
  } catch (error) {
    if (error.errors) {
      // 表单验证失败
      ElMessage.warning('请检查表单填写')
    } else {
      // API 调用失败（已在 request 中提示）
      console.error('提交失败:', error)
    }
  } finally {
    loading.value = false
  }
}
</script>
```

### 3. 全局错误处理

```javascript
// main.js
import { createApp } from 'vue'
import App from './App.vue'

const app = createApp(App)

// ✅ 全局错误处理器
app.config.errorHandler = (err, instance, info) => {
  console.error('全局错误:', err)
  console.error('错误信息:', info)
  
  // 可以上报到错误监控平台
  // reportError(err, info)
}

app.mount('#app')
```

---

## 🔢 错误码定义

### 1. HTTP 状态码

| 状态码 | 含义 | 使用场景 |
|--------|------|----------|
| 200 | 成功 | 请求成功 |
| 201 | 创建成功 | 资源创建成功 |
| 204 | 无内容 | 删除成功(无返回数据) |
| 400 | 参数错误 | 请求参数验证失败 |
| 401 | 未认证 | 未登录或 token 过期 |
| 403 | 无权限 | 没有权限访问资源 |
| 404 | 不存在 | 资源不存在 |
| 409 | 冲突 | 资源状态冲突 |
| 422 | 验证失败 | Pydantic 参数验证失败 |
| 500 | 服务器错误 | 服务器内部错误 |
| 502 | 网关错误 | 上游服务错误 |
| 503 | 服务不可用 | 服务维护中 |

### 2. 业务错误码

```python
# 业务错误码定义
# 格式: 模块(2位) + 错误类型(2位)

# 通用错误 (10xx)
10001 = "操作失败"
10002 = "参数错误"
10003 = "数据不存在"

# 用户模块 (20xx)
20001 = "用户名已存在"
20002 = "邮箱已被注册"
20003 = "密码错误"
20004 = "用户不存在"
20005 = "账号已被禁用"

# 课程模块 (30xx)
30001 = "课程名称已存在"
30002 = "课程不存在"
30003 = "课程已发布，无法修改"
30004 = "课程至少需要一个单元"

# 权限模块 (40xx)
40001 = "无权访问"
40002 = "无权修改"
40003 = "无权删除"

# 使用示例
from app.core.response import error_response

return error_response(
    code=30001,
    message="课程名称已存在",
    status_code=400
)
```

---

## 📝 错误日志记录

### 1. 日志配置

```python
# app/core/logging_config.py
import logging
from logging.handlers import RotatingFileHandler
import os

def setup_logging():
    """配置日志"""
    
    # 创建日志目录
    log_dir = "logs"
    os.makedirs(log_dir, exist_ok=True)
    
    # 日志格式
    formatter = logging.Formatter(
        '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
    )
    
    # 文件处理器(自动轮转)
    file_handler = RotatingFileHandler(
        f"{log_dir}/app.log",
        maxBytes=10*1024*1024,  # 10MB
        backupCount=10
    )
    file_handler.setFormatter(formatter)
    file_handler.setLevel(logging.INFO)
    
    # 错误日志单独文件
    error_handler = RotatingFileHandler(
        f"{log_dir}/error.log",
        maxBytes=10*1024*1024,
        backupCount=10
    )
    error_handler.setFormatter(formatter)
    error_handler.setLevel(logging.ERROR)
    
    # 控制台处理器
    console_handler = logging.StreamHandler()
    console_handler.setFormatter(formatter)
    console_handler.setLevel(logging.DEBUG)
    
    # 配置根日志记录器
    root_logger = logging.getLogger()
    root_logger.setLevel(logging.INFO)
    root_logger.addHandler(file_handler)
    root_logger.addHandler(error_handler)
    root_logger.addHandler(console_handler)
```

### 2. 日志记录规范

```python
import logging

logger = logging.getLogger(__name__)

@router.post("/courses")
def create_course(course: CourseCreate, db: Session = Depends(get_db)):
    try:
        # ✅ 记录关键操作
        logger.info(f"创建课程: {course.title}")
        
        new_course = Course(**course.dict())
        db.add(new_course)
        db.commit()
        
        # ✅ 记录成功操作
        logger.info(f"课程创建成功: id={new_course.id}, uuid={new_course.uuid}")
        
        return success_response(data=new_course.to_dict())
        
    except IntegrityError as e:
        db.rollback()
        # ✅ 记录警告(预期内的错误)
        logger.warning(f"课程名称重复: {course.title}, error: {str(e)}")
        raise HTTPException(status_code=400, detail="课程名称已存在")
        
    except Exception as e:
        db.rollback()
        # ✅ 记录错误(包含堆栈信息)
        logger.error(
            f"创建课程失败: {course.title}",
            exc_info=True  # 包含完整堆栈
        )
        raise HTTPException(status_code=500, detail="创建失败")

# 日志级别使用:
# logger.debug()    - 调试信息
# logger.info()     - 一般信息
# logger.warning()  - 警告(预期内的问题)
# logger.error()    - 错误(需要关注的问题)
# logger.critical() - 严重错误(系统无法运行)
```

### 3. 敏感信息过滤

```python
import logging

logger = logging.getLogger(__name__)

def safe_log_data(data: dict) -> dict:
    """过滤敏感信息"""
    sensitive_fields = ['password', 'token', 'secret', 'api_key']
    safe_data = data.copy()
    
    for field in sensitive_fields:
        if field in safe_data:
            safe_data[field] = '***'
    
    return safe_data

# ✅ 使用
logger.info(f"用户登录: {safe_log_data(user_data)}")

# ❌ 避免记录敏感信息
logger.info(f"用户登录: {user_data}")  # 可能包含密码
```

---

## 💡 最佳实践

### 1. 错误信息设计

```python
# ✅ 好的错误信息:
# - 清晰描述问题
# - 提供解决建议
# - 不暴露敏感信息

raise HTTPException(
    status_code=400,
    detail="课程标题已存在，请使用其他标题"
)

raise HTTPException(
    status_code=403,
    detail="只有教师可以创建课程"
)

# ❌ 不好的错误信息:
raise HTTPException(status_code=500, detail="Error")  # 不清晰
raise HTTPException(status_code=400, detail=str(e))   # 可能暴露技术细节
```

### 2. 错误分类处理

```python
@router.post("/courses")
def create_course(course: CourseCreate, db: Session = Depends(get_db)):
    try:
        # 业务逻辑
        pass
        
    except HTTPException:
        # ✅ HTTPException 直接抛出，不需要额外处理
        raise
        
    except IntegrityError as e:
        # ✅ 数据库约束错误 - 转换为友好提示
        db.rollback()
        raise HTTPException(status_code=400, detail="数据已存在")
        
    except ValueError as e:
        # ✅ 参数值错误 - 返回 400
        raise HTTPException(status_code=400, detail=str(e))
        
    except Exception as e:
        # ✅ 未知错误 - 记录日志并返回通用错误
        db.rollback()
        logger.error(f"创建课程失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail="创建失败")
```

### 3. 优雅降级

```javascript
// ✅ 前端优雅降级
async function loadCourses() {
  try {
    const data = await getCourses()
    courses.value = data.items
  } catch (error) {
    // 加载失败，显示空状态
    courses.value = []
    // 不阻断用户操作
  }
}

// ✅ 提供默认值
const courseDetail = ref({
  title: '加载中...',
  description: '',
  units: []
})
```

### 4. 错误监控

```python
# 集成错误监控服务(如 Sentry)
import sentry_sdk

sentry_sdk.init(
    dsn="your-dsn",
    traces_sample_rate=1.0
)

# 自动捕获未处理的异常
```

---

## 📚 参考资源

- [FastAPI 错误处理文档](https://fastapi.tiangolo.com/tutorial/handling-errors/)
- [HTTP 状态码](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Status)
- [Python logging 文档](https://docs.python.org/zh-cn/3/library/logging.html)

---

**记住**: 良好的错误处理让系统更健壮，用户体验更友好！
