# 测试规范

> 本文档定义了 CodeHubot 项目的测试规范，包括单元测试、集成测试和测试覆盖率要求。

## 📋 目录

- [测试原则](#测试原则)
- [测试类型](#测试类型)
- [后端测试规范](#后端测试规范)
- [前端测试规范](#前端测试规范)
- [测试覆盖率要求](#测试覆盖率要求)
- [最佳实践](#最佳实践)

---

## 🎯 测试原则

### 1. 测试金字塔

```
       /\
      /  \      E2E测试 (少量)
     /    \     - 关键业务流程
    /------\    
   /        \   集成测试 (适量)
  /          \  - API接口测试
 /------------\ - 组件集成测试
/              \
\--------------/ 单元测试 (大量)
                - 函数测试
                - 工具函数测试
```

### 2. 测试编写原则

- **F.I.R.S.T 原则**:
  - **Fast**: 测试运行要快
  - **Independent**: 测试之间相互独立
  - **Repeatable**: 可重复执行
  - **Self-Validating**: 自动验证结果
  - **Timely**: 及时编写测试

### 3. 测试命名规范

```python
# Python 测试命名: test_{function}_{scenario}_{expected}
def test_create_course_with_valid_data_should_success()
def test_create_course_with_duplicate_name_should_raise_error()
def test_get_course_list_with_pagination_should_return_correct_items()
```

```javascript
// JavaScript 测试命名: {function} should {expected} when {scenario}
describe('CourseAPI', () => {
  it('should create course when data is valid', () => {})
  it('should throw error when course name is duplicate', () => {})
  it('should return correct items when pagination is applied', () => {})
})
```

---

## 🔬 测试类型

### 1. 单元测试

测试单个函数或方法的功能。

```python
# 测试工具函数
def test_format_datetime():
    """测试日期时间格式化"""
    dt = datetime(2024, 1, 1, 12, 0, 0)
    result = format_datetime(dt)
    assert result == "2024-01-01 12:00:00"

# 测试模型方法
def test_course_to_dict():
    """测试课程模型转字典"""
    course = Course(title="Python基础", status="draft")
    result = course.to_dict()
    
    assert result["title"] == "Python基础"
    assert result["status"] == "draft"
```

### 2. 集成测试

测试多个模块或组件的交互。

```python
# 测试 API 接口
def test_api_create_course(client, db_session):
    """测试创建课程API"""
    data = {
        "title": "Python基础",
        "description": "适合初学者"
    }
    
    response = client.post("/api/courses", json=data)
    
    assert response.status_code == 200
    assert response.json()["code"] == 200
    assert response.json()["data"]["title"] == "Python基础"
```

### 3. 端到端测试 (E2E)

测试完整的业务流程。

```javascript
// Cypress E2E 测试
describe('Course Management', () => {
  it('should complete course creation flow', () => {
    // 1. 登录
    cy.visit('/login')
    cy.get('[name=username]').type('teacher')
    cy.get('[name=password]').type('password')
    cy.get('[type=submit]').click()
    
    // 2. 创建课程
    cy.visit('/courses/create')
    cy.get('[name=title]').type('Python基础')
    cy.get('[name=description]').type('适合初学者')
    cy.get('[type=submit]').click()
    
    // 3. 验证结果
    cy.contains('创建成功')
    cy.url().should('include', '/courses')
  })
})
```

---

## 🐍 后端测试规范

### 1. 测试环境配置

```python
# tests/conftest.py
import pytest
from fastapi.testclient import TestClient
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker

from app.main import app
from app.core.database import Base, get_db

# 测试数据库配置
SQLALCHEMY_TEST_DATABASE_URL = "sqlite:///./test.db"

engine = create_engine(
    SQLALCHEMY_TEST_DATABASE_URL,
    connect_args={"check_same_thread": False}
)
TestingSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)


@pytest.fixture(scope="function")
def db_session():
    """创建测试数据库会话"""
    Base.metadata.create_all(bind=engine)
    db = TestingSessionLocal()
    try:
        yield db
    finally:
        db.close()
        Base.metadata.drop_all(bind=engine)


@pytest.fixture(scope="function")
def client(db_session):
    """创建测试客户端"""
    def override_get_db():
        try:
            yield db_session
        finally:
            pass
    
    app.dependency_overrides[get_db] = override_get_db
    
    with TestClient(app) as test_client:
        yield test_client


@pytest.fixture
def sample_course(db_session):
    """创建示例课程"""
    course = Course(
        uuid="test-uuid-123",
        title="测试课程",
        description="这是一个测试课程",
        status="draft"
    )
    db_session.add(course)
    db_session.commit()
    db_session.refresh(course)
    return course
```

### 2. API 测试示例

```python
# tests/test_api/test_courses.py
import pytest
from fastapi import status


class TestCourseAPI:
    """课程 API 测试"""
    
    def test_get_courses_list(self, client, sample_course):
        """测试获取课程列表"""
        response = client.get("/api/courses?page=1&page_size=20")
        
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        
        assert data["code"] == 200
        assert "items" in data["data"]
        assert "total" in data["data"]
        assert len(data["data"]["items"]) > 0
    
    def test_create_course_success(self, client, db_session):
        """测试创建课程 - 成功"""
        data = {
            "title": "Python基础",
            "description": "适合初学者",
            "status": "draft"
        }
        
        response = client.post("/api/courses", json=data)
        
        assert response.status_code == status.HTTP_200_OK
        result = response.json()
        
        assert result["code"] == 200
        assert result["data"]["title"] == "Python基础"
        assert "uuid" in result["data"]
    
    def test_create_course_with_duplicate_name(self, client, sample_course):
        """测试创建课程 - 名称重复"""
        data = {
            "title": sample_course.title,  # 重复的名称
            "description": "测试描述"
        }
        
        response = client.post("/api/courses", json=data)
        
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        result = response.json()
        assert result["code"] == 400
    
    def test_get_course_detail_success(self, client, sample_course):
        """测试获取课程详情 - 成功"""
        response = client.get(f"/api/courses/{sample_course.uuid}")
        
        assert response.status_code == status.HTTP_200_OK
        result = response.json()
        
        assert result["code"] == 200
        assert result["data"]["uuid"] == sample_course.uuid
        assert result["data"]["title"] == sample_course.title
    
    def test_get_course_detail_not_found(self, client):
        """测试获取课程详情 - 不存在"""
        response = client.get("/api/courses/non-existent-uuid")
        
        assert response.status_code == status.HTTP_404_NOT_FOUND
        result = response.json()
        assert result["code"] == 404
    
    def test_update_course_success(self, client, sample_course):
        """测试更新课程 - 成功"""
        data = {
            "title": "更新后的标题",
            "description": "更新后的描述"
        }
        
        response = client.put(f"/api/courses/{sample_course.uuid}", json=data)
        
        assert response.status_code == status.HTTP_200_OK
        result = response.json()
        
        assert result["code"] == 200
        assert result["data"]["title"] == "更新后的标题"
    
    def test_delete_course_success(self, client, sample_course):
        """测试删除课程 - 成功"""
        response = client.delete(f"/api/courses/{sample_course.uuid}")
        
        assert response.status_code == status.HTTP_200_OK
        result = response.json()
        assert result["code"] == 200
```

### 3. 业务逻辑测试

```python
# tests/test_services/test_course_service.py
import pytest
from app.services.course_service import CourseService


class TestCourseService:
    """课程服务测试"""
    
    def test_validate_course_data(self):
        """测试课程数据验证"""
        service = CourseService()
        
        # 有效数据
        valid_data = {"title": "Python基础", "description": "测试"}
        assert service.validate_course_data(valid_data) is True
        
        # 无效数据 - 标题太短
        invalid_data = {"title": "P", "description": "测试"}
        with pytest.raises(ValueError):
            service.validate_course_data(invalid_data)
    
    def test_calculate_course_progress(self, sample_course):
        """测试计算课程进度"""
        service = CourseService()
        
        # 创建单元和任务
        # ... 准备数据
        
        progress = service.calculate_course_progress(sample_course)
        
        assert 0 <= progress <= 100
        assert isinstance(progress, (int, float))
```

### 4. 模型测试

```python
# tests/test_models/test_course.py
import pytest
from datetime import datetime
from app.models.pbl import PBLCourse


class TestCourseModel:
    """课程模型测试"""
    
    def test_create_course(self, db_session):
        """测试创建课程"""
        course = PBLCourse(
            uuid="test-uuid",
            title="测试课程",
            status="draft"
        )
        
        db_session.add(course)
        db_session.commit()
        
        assert course.id is not None
        assert course.created_at is not None
    
    def test_course_to_dict(self):
        """测试课程转字典"""
        course = PBLCourse(
            uuid="test-uuid",
            title="测试课程",
            status="draft"
        )
        
        result = course.to_dict()
        
        assert result["uuid"] == "test-uuid"
        assert result["title"] == "测试课程"
        assert result["status"] == "draft"
    
    def test_course_soft_delete(self, db_session):
        """测试课程软删除"""
        course = PBLCourse(title="测试课程")
        db_session.add(course)
        db_session.commit()
        
        # 软删除
        course.is_deleted = True
        course.deleted_at = datetime.now()
        db_session.commit()
        
        # 验证
        assert course.is_deleted is True
        assert course.deleted_at is not None
```

---

## 🎨 前端测试规范

### 1. 单元测试（Vitest）

```javascript
// tests/unit/utils/format.spec.js
import { describe, it, expect } from 'vitest'
import { formatDate, formatCurrency } from '@/utils/format'

describe('formatDate', () => {
  it('should format date correctly', () => {
    const date = new Date('2024-01-01 12:00:00')
    const result = formatDate(date, 'YYYY-MM-DD')
    expect(result).toBe('2024-01-01')
  })
  
  it('should handle null date', () => {
    const result = formatDate(null)
    expect(result).toBe('-')
  })
})

describe('formatCurrency', () => {
  it('should format currency with symbol', () => {
    const result = formatCurrency(1234.56)
    expect(result).toBe('¥1,234.56')
  })
})
```

### 2. 组件测试

```javascript
// tests/unit/components/CourseCard.spec.js
import { describe, it, expect } from 'vitest'
import { mount } from '@vue/test-utils'
import CourseCard from '@/components/CourseCard.vue'

describe('CourseCard', () => {
  it('should render course title', () => {
    const wrapper = mount(CourseCard, {
      props: {
        course: {
          title: 'Python基础',
          description: '适合初学者'
        }
      }
    })
    
    expect(wrapper.text()).toContain('Python基础')
  })
  
  it('should emit click event when card is clicked', async () => {
    const wrapper = mount(CourseCard, {
      props: {
        course: { title: 'Python基础' }
      }
    })
    
    await wrapper.trigger('click')
    
    expect(wrapper.emitted()).toHaveProperty('click')
  })
})
```

### 3. API Mock 测试

```javascript
// tests/unit/api/courses.spec.js
import { describe, it, expect, vi } from 'vitest'
import { getCourses, createCourse } from '@/modules/pbl/api/courses'
import request from '@/utils/request'

// Mock request 模块
vi.mock('@/utils/request')

describe('Course API', () => {
  it('should get courses list', async () => {
    const mockData = {
      items: [
        { id: 1, title: 'Course 1' },
        { id: 2, title: 'Course 2' }
      ],
      total: 2
    }
    
    request.mockResolvedValue(mockData)
    
    const result = await getCourses({ page: 1, pageSize: 20 })
    
    expect(result).toEqual(mockData)
    expect(request).toHaveBeenCalledWith({
      url: '/api/courses',
      method: 'get',
      params: { page: 1, pageSize: 20 }
    })
  })
  
  it('should create course', async () => {
    const courseData = {
      title: 'New Course',
      description: 'Test'
    }
    const mockResponse = { id: 1, ...courseData }
    
    request.mockResolvedValue(mockResponse)
    
    const result = await createCourse(courseData)
    
    expect(result).toEqual(mockResponse)
  })
})
```

### 4. E2E 测试（Cypress）

```javascript
// cypress/e2e/course-management.cy.js
describe('Course Management', () => {
  beforeEach(() => {
    // 登录
    cy.login('teacher', 'password')
  })
  
  it('should create a new course', () => {
    // 访问课程列表页
    cy.visit('/courses')
    
    // 点击创建按钮
    cy.contains('创建课程').click()
    
    // 填写表单
    cy.get('[name=title]').type('Python基础')
    cy.get('[name=description]').type('适合初学者')
    
    // 提交
    cy.get('[type=submit]').click()
    
    // 验证
    cy.contains('创建成功')
    cy.contains('Python基础').should('be.visible')
  })
  
  it('should edit course', () => {
    cy.visit('/courses')
    
    // 点击编辑按钮
    cy.contains('Python基础')
      .parents('.course-item')
      .find('.edit-btn')
      .click()
    
    // 修改标题
    cy.get('[name=title]')
      .clear()
      .type('Python进阶')
    
    // 保存
    cy.get('[type=submit]').click()
    
    // 验证
    cy.contains('更新成功')
    cy.contains('Python进阶').should('be.visible')
  })
})
```

---

## 📊 测试覆盖率要求

### 1. 覆盖率目标

| 类型 | 目标覆盖率 | 说明 |
|------|-----------|------|
| **核心业务逻辑** | ≥ 80% | 关键功能必须充分测试 |
| **API 接口** | ≥ 60% | 主要接口需要测试 |
| **工具函数** | ≥ 90% | 工具函数易于测试，要求高覆盖 |
| **UI 组件** | ≥ 50% | 重点组件需要测试 |
| **整体项目** | ≥ 60% | 总体覆盖率要求 |

### 2. 运行覆盖率测试

```bash
# Python 后端
pytest --cov=app --cov-report=html tests/

# JavaScript 前端
npm run test:coverage
```

### 3. 查看覆盖率报告

```bash
# 后端: 打开 htmlcov/index.html
open htmlcov/index.html

# 前端: 打开 coverage/index.html
open coverage/index.html
```

---

## 💡 最佳实践

### 1. 测试数据管理

```python
# ✅ 使用 Fixture 管理测试数据
@pytest.fixture
def sample_courses(db_session):
    """创建多个示例课程"""
    courses = [
        Course(title=f"课程{i}", status="draft")
        for i in range(5)
    ]
    db_session.add_all(courses)
    db_session.commit()
    return courses

# ✅ 使用 Factory 创建测试数据
class CourseFactory:
    @staticmethod
    def create(title="默认课程", **kwargs):
        return Course(title=title, **kwargs)

# ❌ 避免硬编码测试数据
def test_example():
    course = Course(title="课程1", description="描述1", ...)  # 太冗长
```

### 2. 测试独立性

```python
# ✅ 每个测试独立，不依赖其他测试
def test_create_course(db_session):
    course = Course(title="测试课程")
    db_session.add(course)
    db_session.commit()
    assert course.id is not None

def test_get_course(db_session):
    # 创建自己的数据，不依赖上一个测试
    course = Course(title="测试课程2")
    db_session.add(course)
    db_session.commit()
    
    result = db_session.query(Course).filter_by(id=course.id).first()
    assert result is not None
```

### 3. 测试命名清晰

```python
# ✅ 清晰的测试命名
def test_create_course_with_valid_data_should_success():
    pass

def test_create_course_with_empty_title_should_raise_error():
    pass

# ❌ 模糊的测试命名
def test_course_1():
    pass

def test_create():
    pass
```

### 4. 边界测试

```python
# ✅ 测试边界情况
def test_pagination_with_empty_list():
    """测试空列表分页"""
    pass

def test_pagination_with_one_item():
    """测试只有一条数据的分页"""
    pass

def test_pagination_with_exact_page_size():
    """测试数据量正好等于页大小"""
    pass

def test_pagination_with_last_page():
    """测试最后一页"""
    pass
```

### 5. 异常测试

```python
# ✅ 测试异常情况
def test_create_course_with_invalid_data_should_raise_error():
    """测试无效数据应该抛出异常"""
    with pytest.raises(ValueError):
        create_course({"title": ""})  # 空标题

def test_get_non_existent_course_should_raise_404():
    """测试获取不存在的课程应该返回404"""
    response = client.get("/api/courses/non-existent")
    assert response.status_code == 404
```

---

## 🚀 测试执行

### 1. 运行所有测试

```bash
# Python 后端
pytest

# JavaScript 前端
npm run test
```

### 2. 运行特定测试

```bash
# 运行特定文件
pytest tests/test_api/test_courses.py

# 运行特定测试
pytest tests/test_api/test_courses.py::test_create_course_success

# 运行匹配的测试
pytest -k "course"
```

### 3. 持续集成

```yaml
# .github/workflows/test.yml
name: Tests

on: [push, pull_request]

jobs:
  test:
    runs-on: ubuntu-latest
    
    steps:
    - uses: actions/checkout@v2
    
    - name: Set up Python
      uses: actions/setup-python@v2
      with:
        python-version: 3.9
    
    - name: Install dependencies
      run: |
        pip install -r requirements.txt
        pip install pytest pytest-cov
    
    - name: Run tests
      run: pytest --cov=app tests/
    
    - name: Upload coverage
      run: codecov
```

---

## 📚 参考资源

- [Pytest 文档](https://docs.pytest.org/)
- [Vitest 文档](https://vitest.dev/)
- [Cypress 文档](https://www.cypress.io/)
- [Vue Test Utils](https://test-utils.vuejs.org/)

---

**记住**: 测试是代码质量的保障，投入时间编写测试会大大减少后期的 bug 修复成本！
